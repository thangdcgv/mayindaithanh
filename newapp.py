...
def get_sql_engine():
    conf = st.secrets["connections"]["supabase_sql"]
...
# --- TAB 1: GỬI ĐƠN LẮP ĐẶT (TỐI ƯU CHO COOKIE) ---
    with tabs[0]:
        ...
        target_user = user
        if role in ["Manager", "Admin", "System Admin"]:
...
# --- TAB 2: DUYỆT ĐƠN (CHỈ ADMIN/SYSTEM ADMIN/MANAGER) ---
    if role in ["Admin", "System Admin", "Manager", "User"]:
        with tabs[1]:
            ...
            # Tối ưu 1: Chỉ lấy các cột cần thiết, tránh tải 'hinh_anh' nếu không cần hoặc ảnh quá lớn
            # Lưu ý: Nếu bạn đã nén ảnh <100KB thì có thể để lại, nếu ảnh gốc thì nên tách ra.
            try:
                query = supabase.table("cham_cong") \
                    .select("id, thoi_gian, so_hoa_don, noi_dung, quang_duong, combo, thanh_tien, hinh_anh, trang_thai, username, quan_tri_vien(ho_ten)") \
                    .eq("trang_thai", "Chờ duyệt")
                # PHÂN QUYỀN HIỂN THỊ DỮ LIỆU
                # Nếu không phải các quyền quản trị, thì lọc theo chính username của người đang đăng nhập
                if role not in ["Admin", "System Admin", "Manager"]:
                    query = query.eq("username", user_hien_tai)
                res = query.order("thoi_gian", desc=True).execute()
                df_p = pd.DataFrame(res.data)
                
                if not df_p.empty:
                    # Tối ưu 2: Xử lý dữ liệu hàng loạt thay vì xử lý trong vòng lặp for
                    df_p['ho_ten_nv'] = df_p['quan_tri_vien'].apply(lambda x: x['ho_ten'] if x else "N/A")
                    df_p['dt_raw'] = pd.to_datetime(df_p['thoi_gian'])
                    
                    # Xử lý múi giờ tập trung
                    vn_tz = pytz.timezone('Asia/Ho_Chi_Minh')
                    def format_time(dt):
                        if dt.tzinfo is None:
                            return dt.replace(tzinfo=pytz.UTC).astimezone(vn_tz).strftime('%d/%m/%Y %H:%M')
                        return dt.astimezone(vn_tz).strftime('%d/%m/%Y %H:%M')
                    
                    df_p['time_display'] = df_p['dt_raw'].apply(format_time)
            except Exception as e:
            ...
            if df_p.empty:
                ...
                for _, r in df_p.iterrows():
                    ...
                    with st.expander(expander_title):
                        ...
                        with cr:
                            # Tối ưu 3: Hiển thị ảnh an toàn
                            img_data = r.get("hinh_anh")
                            if img_data:
                                # Nếu chuỗi quá ngắn (lỗi data) thì không xử lý
                                if len(img_data) > 100:
                                    if not img_data.startswith("data:image"):
                                        img_data = f"data:image/jpeg;base64,{img_data}"
                                    st.image(img_data, caption=f"HĐ {r['so_hoa_don']}", use_container_width=True)
                                else:
        ...
        with tabs[-1]:
            ...
            try:
                ...
                res = supabase.table("cham_cong") \
                    .select("id, username, thoi_gian, so_hoa_don, noi_dung, quang_duong, combo, thanh_tien, trang_thai, ghi_chu_duyet, quan_tri_vien(ho_ten)") \
                    .execute()
                ...
                if not df_raw.empty:
                    ...
                    # Chuyển đổi múi giờ VN tập trung
                    df_raw["thoi_gian"] = pd.to_datetime(df_raw["thoi_gian"])
                    df_raw["Thời Gian"] = df_raw["thoi_gian"].dt.tz_convert('Asia/Ho_Chi_Minh').dt.tz_localize(None)
                        ...
                        with st.expander("📊 Tra cứu chi tiết và Xuất báo cáo", expanded=False):
                            ...
                            # --- 2. Hàm xử lý logic chuyển trang (Callback) ---
                                ...
                                if df_display.empty:
                                    ...
                                    # A. Định dạng múi giờ Việt Nam (Tối ưu logic chuyển đổi)
                                    if 'Thời Gian' in df_view.columns:
                                        df_view['Thời Gian'] = pd.to_datetime(df_view['Thời Gian'])
                                        # Chuyển đổi múi giờ tập trung (Vectorized operation)
                                        df_view['Thời Gian'] = (
                                            df_view['Thời Gian']
                                            .dt.tz_localize('UTC', ambiguous='infer').dt.tz_convert('Asia/Ho_Chi_Minh')
                                            if df_view['Thời Gian'].dt.tz is None 
                                            else df_view['Thời Gian'].dt.tz_convert('Asia/Ho_Chi_Minh')
                                        )
                                    ...
                                    df_final = df_view[[c for c in desired_columns if c in df_view.columns]]
    ...
    for i, tab_name in enumerate(list_tabs):
        ...
        #--- 4. BẢO TRÌ HỆ THỐNG ---
                ...
                with st.expander("🔥 Dọn dẹp dữ liệu"):
                    ...
                    if st.button("🗑️ THỰC HIỆN XÓA", type="primary", disabled=not confirm_reset, use_container_width=True):
                        try:
